...
Go to: **Google SecOps Console** → **Query** section

#### 2. Create a New Query

Click: **New Query** → **Native Query**
...
outcome:
  $rule_count = count_distinct(detection.id)
  $log_type = detection.log_type
  $event_type = detection.event_type
  $product_event = detection.product_event_type
  $trigger_count = count_distinct(detection.detection_id)
  $event_count = count_distinct(metadata.id)
  $estimated_gb = ($event_count * 1.5) / (1024 * 1024 * 1024)
  $day = detection.event_date
...
Go to: **Google SecOps Console** → **Query** → **New Query** → **Native Query**
...
- First row contains headers
- Has columns: `log_type`, `event_type`, `product_event_type`, `event_count`
...
Open the file and verify:
- First row contains headers
- Has columns: `log_type`, `week`, `event_count`, `week_gigabytes`
